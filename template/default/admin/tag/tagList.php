...
function soloDel( str ){
	...
	str = {id:str};
	$.post('<?=URL?>index.php/admin/tag/tagDelete', str, function(data){
		alert(data);
		var page = $('#page').html();
		window.location.href = '<?=URL?>index.php/admin/tag/tagList?page='+page;
	});
	return false;
}
//批量删除
...
<ul id="container">
  ...
  <li id="tips">
   	<a href="<?=URL?>index.php/admin/tag/tagEdit"><span style="font-size:14px;font-weight:bold;"> >>发布品牌</span></a>
  </li>
</ul>
    ...
    <tr>
        ...
        <td>
          <a href="<?=URL?>index.php/admin/tag/tagEdit?id=<?=$v->id?>">修改</a>
    ...
    <?=$tags->getHtml("?sort=$type")?>
</body>
<script>
//更改是否显示首页
$(".isindex").click(function(){
	var current = $(this);
	$.get("<?php echo URL; ?>index.php/admin/tag/changeInd",{id:$(current).attr("id"),vid:$(current).attr("name")},function(data){
		if(data==1){
