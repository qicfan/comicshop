...
function edit(id) {
	...
	str = {id:id, name:name};
	$.post('<?=URL?>index.php/admin/supply/producer?act=edit', str, function(data){
...
function confirmDel() {
  ...
  <ul id="container">
    <li id="header">
      <h1>供货商管理</h1>
      <div class="link"></div>
      <div id="desc">生产商管理</div>
    </li>
  <li id="tips">
<form action="<?=URL?>index.php/admin/supply/producer?act=add" method="post">
	添加新生产商：
...
<tr>
	...
	<td><input type="text" class="input" id="this<?=$v->id?>" value="<?=$v->pname?>" style="border:#DDDDDD 1px solid;" /></td>
    <td>
    <a href="javascript:void(0);" onclick="return edit('<?=$v->id?>');">更新名称</a>
     | <a href="<?=URL?>index.php/admin/supply/producer?act=del&id=<?=$v->id?>" onclick="return confirmDel();">移除</a>
    </td>
</tr>
<?php
}
